 ...
 * way for the <code>Formatter</code> to store the start tag for an XML element.
...
class OutputBuffer {
	 ...
	 * <code>OutputBuffer</code> stores 16 characters before a resize is needed
	 * to append extra characters.
	 */
	OutputBuffer() {
	...
	/**
	 * This will add a <code>char</code> to the end of the buffer. The buffer
	 * will not overflow with repeated uses of the <code>append</code>, it uses
	 * an <code>ensureCapacity</code> method which will allow the buffer to
	 * dynamically grow in size to accommodate more characters.
	 * 
	 * @param ch
	...
	/**
	 * This will add a <code>String</code> to the end of the buffer. The buffer
	 * will not overflow with repeated uses of the <code>append</code>, it uses
	 * an <code>ensureCapacity</code> method which will allow the buffer to
	 * dynamically grow in size to accommodate large string objects.
	 * 
	 * @param value
	...
	/**
	 * This will add a <code>char</code> array to the buffer. The buffer will
	 * not overflow with repeated uses of the <code>append</code>, it uses an
	 * <code>ensureCapacity</code> method which will allow the buffer to
	 * dynamically grow in size to accommodate large character arrays.
	 * 
	 * @param value
	 ...
	 * specified <code>Writer</code> object nothing is written out. This allows
	 * XML elements to be removed.
	 */
	public void clear() {
		text.setLength(0);
	}
